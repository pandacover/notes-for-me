...
const SignUp = () => {
	const [[name, email, password], setCredentials] = React.useState([
		"",
		"",
		"",
	]);
	...
	const onUserCreate = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			setLoading(true);
			const { data: response } = await Auth.registerUser({
				name,
				email,
				password,
				createdAt: new Date(),
		...
		>
			<div>
				...
				<input
					...
					onChange={(e) =>
						setCredentials([e.currentTarget.value, email, password])
					}
				/>
			</div>
			<div>
				...
				<input
					...
					placeholder='email'
					onChange={(e) =>
						setCredentials([name, e.currentTarget.value, password])
					}
				/>
			</div>
			<div>
				...
				<input
					...
					onChange={(e) => setCredentials([name, email, e.currentTarget.value])}
				/>
			</div>
			<div className='h-10'>
				<button
